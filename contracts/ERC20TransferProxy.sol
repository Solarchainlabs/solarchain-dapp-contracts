...
contract ERC20TransferProxy is Ownable, IERC20TransferProxy {
    ...
    function erc20safeTransferFrom(IERC20Upgradeable token, address from, address to, uint256 value) override external {
        require(_dappSet.contains(msg.sender), "!allowed dapp");
        uint256 oldBalance = token.balanceOf(to);
        require(token.transferFrom(from, to, value), "Failure while transferring");
        require(token.balanceOf(to) == oldBalance + value, "Not enough token was transfered");        
    }
}
