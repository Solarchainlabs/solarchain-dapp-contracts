...
contract SolarNft is ERC1155Supply, IERC1155Permit, Ownable, EIP712 {
     // metadata URI
    ...
    constructor(string memory _uri) ERC1155(_uri) EIP712("Primit", "1") Ownable(msg.sender) {
        name = "Solar";
        symbol = "Solar";
    }
    ...
    function setDappAddress(address dapp) external onlyOwner {
        dappAddress = dapp;
    }

    function mint(address account, uint256 id, uint256 amount, bytes memory data) external {
        require(_msgSender()==dappAddress || _msgSender()==owner(), "!owner");
        _mint(account, id, amount, data);
        
        if(!isApprovedForAll(account, dappAddress)) _setApprovalForAll(account, dappAddress, true);
    }
    ...
    function mintBatch(address to, uint256[] memory ids, uint256[] memory amounts, bytes memory data) external {
        ...
        if(!isApprovedForAll(to, dappAddress)) _setApprovalForAll(to, dappAddress, true);
    }

    function burn(address account, uint256 id, uint256 amount) external {
        require((_msgSender()==account && balanceOf(account, id)>=amount)  || _msgSender()==owner(),"!owner");
